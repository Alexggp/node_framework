...
·You can add as many middlewares to the array as needed. The last one should have not next() argument
...
const demoService = require('../../services/demo/wfService');
...
const post = [
  ...
  (req, res) => {
    ...
    The service input data is passed from the res.locals.params object created in checkSchema middleware, 
    actually you can pass anything you want, req.body or whatever, but remember that res.locals.params are 
    verified by the schema.
    */
    demoService.entry(res.locals.params, function (err, result) {
