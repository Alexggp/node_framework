/*
	...
	It is implemented with async waterfall:
		...
		·The first parameter is the error parameter. 
				·If an error is returned by a transaction you should call calback(error), passing error object (or not null) as first argument .
				·When callback(error) is called the waterfall brakes and returns the error.
		·In no error case, first argument (error argument) is null and the other arguments in the callback call are the input parameters of the next function. 
...
function adapter(data, sentence, callback){
	/*
		Here you can see how data and sentence objects are passed from "asyncTransaction" function callback: 
			callback(null, data, sentence);
